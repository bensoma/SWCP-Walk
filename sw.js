...
const PRECACHE      = `swcp-redesign-precache-${VERSION}`;
const RUNTIME_CACHE = `swcp-redesign-runtime-${VERSION}`;
...
/**
 * Third-party runtime libraries — precached best-effort into the runtime cache
 * at install so maps, PDF export, ZIP backup and charts work offline even if
 * the user never opened those features while online. Failures are swallowed;
 * the lazy loaders in loaders.js fall back to the network as before.
 */
const PRECACHE_CDN = [
...
self.addEventListener('install', event => {
  event.waitUntil((async () => {
    ...
    // CDN libraries are best-effort too — these URLs are versioned/immutable,
    // so plain fetch (browser cache allowed) is fine.
    const runtime = await caches.open(RUNTIME_CACHE);
    await Promise.all(PRECACHE_CDN.map(async url => {
      try {
        const res = await fetch(url);
        if (res.ok) await runtime.put(url, res);
      } catch (_) { /* offline or CDN hiccup — lazy loaders fall back */ }
    }));
